...
case "$PATCH_SET" in
    core)
        PATCHES=("$PATCH_FILE" "$COMM_DIAGNOSTICS_PATCH_FILE")
        ;;
    debug|all)
        PATCHES=("$PATCH_FILE" "$COMM_DIAGNOSTICS_PATCH_FILE" "$TRACE_PATCH_FILE" "$TEXTURE_TRACE_PATCH_FILE" "$TEXTURE_SOURCE_TRACE_PATCH_FILE" "$PALETTE_TRACE_PATCH_FILE" "$TEXTURE_COMMAND_TRACE_PATCH_FILE" "$GEOMETRY_OBJECT_TRACE_PATCH_FILE" "$GEOMETRY_MATRIX_TRACE_PATCH_FILE" "$GEOMETRY_POLYGON_TRACE_PATCH_FILE")
        ;;
    texture-trace)
        PATCHES=("$PATCH_FILE" "$COMM_DIAGNOSTICS_PATCH_FILE" "$TEXTURE_TRACE_PATCH_FILE" "$TEXTURE_SOURCE_TRACE_PATCH_FILE")
        ;;
    *)
        printf 'error: unknown VON_MAME_PATCH_SET=%s (expected core, texture-trace, debug, or all)\n' "$PATCH_SET" >&2
        exit 1
        ;;
esac
