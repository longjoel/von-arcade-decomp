/* Shared callback byte-map finalizer recovered from i960 0x85ef8-0x85f58. */

#include <stdint.h>

typedef uint32_t u32;

struct recovered_scheduler_callback_byte_map_finalize_85ef8 {
    u32 writes;
    uint8_t map_after[32];
};

struct recovered_scheduler_callback_byte_map_finalize_85ef8
recovered_scheduler_callback_byte_map_finalize_85ef8(
    const uint8_t object_200_bytes[32], const uint16_t object_204_words[32],
    const uint8_t map_before[32], u32 value_504e42)
{
    struct recovered_scheduler_callback_byte_map_finalize_85ef8 out;
    u32 index;
