/* Cursor-home dispatch recovered from i960 0x1ef70-0x1f00c. */
#include <stdint.h>
...
static void recovered_home_dispatch_variant_plan(
    u32 selector, u32 row_home, struct recovered_home_dispatch_plan *plan)
{
                                  ...
                                  struct recovered_home_dispatch_plan *plan)
{
    ...
    recovered_home_dispatch_variant_plan(selector, 18U, plan);
}

/* The adjacent 0x1efc0 entry has the same dispatch contract with row home 2. */
void recovered_home_dispatch_1efc0_plan(
    u32 selector, struct recovered_home_dispatch_plan *plan)
{
    recovered_home_dispatch_variant_plan(selector, 2U, plan);
}
