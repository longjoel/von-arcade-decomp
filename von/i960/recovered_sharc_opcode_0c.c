...
static float recovered_sharc_opcode_0c_float(u32 bits)
{
    union { u32 bits; float value; } input;
    input.bits = bits;
    return input.value;
}

static u32 recovered_sharc_opcode_0c_bits(float value)
{
    union { u32 bits; float value; } output;
    output.value = value;
    return output.bits;
}
...
{
    u32 bits = recovered_sharc_opcode_0c_bits(value);
    float r = recovered_sharc_opcode_0c_float(recovered_sharc_rsqrts_seed(bits));
...
{
    float x = recovered_sharc_opcode_0c_float(input[0]);
    float y = recovered_sharc_opcode_0c_float(input[1]);
    float z = recovered_sharc_opcode_0c_float(input[2]);
    float arithmetic_x = recovered_sharc_opcode_0c_is_denormal(input[0])
        ? recovered_sharc_opcode_0c_float(input[0] & 0x80000000U) : x;
    float arithmetic_y = recovered_sharc_opcode_0c_is_denormal(input[1])
        ? recovered_sharc_opcode_0c_float(input[1] & 0x80000000U) : y;
    float arithmetic_z = recovered_sharc_opcode_0c_is_denormal(input[2])
        ? recovered_sharc_opcode_0c_float(input[2] & 0x80000000U) : z;
    ...
    if (recovered_sharc_opcode_0c_is_infinity(input[0]) ||
        ...
        const float infinity_seed = recovered_sharc_opcode_0c_float(0x1f800000U);
        ...
        for (int lane = 0; lane < 3; ++lane) {
            if (recovered_sharc_opcode_0c_is_infinity(values[lane]))
                ...
                output[lane] = recovered_sharc_opcode_0c_bits(lanes[lane] * infinity_seed);
        }
        return;
    }
    ...
    output[0] = recovered_sharc_opcode_0c_bits(x * reciprocal_length);
    output[1] = recovered_sharc_opcode_0c_bits(y * reciprocal_length);
    output[2] = recovered_sharc_opcode_0c_bits(z * reciprocal_length);
}
