/* Plane-0 attributed glyph emitter recovered from i960 0x1cea0-0x1cf3c. */
#include <stdint.h>

typedef uint32_t u32;
typedef int32_t s32;

struct recovered_glyph_emit_p0a_plan {
    u32 masked_byte;
    s32 biased_byte;
    u32 zeroed;
    u32 glyph_table;
...
void recovered_glyph_emit_p0a_plan(u32 byte, u32 column, u32 row,
                                   struct recovered_glyph_emit_p0a_plan *plan)
{
    u32 tile_slot;
    s32 shifted;
    ...
    /* Distinct table from the 0x2ea0fd0 family used by the other three. */
    plan->glyph_table = 0x02ea10d0U;
    plan->table_index = plan->zeroed ? 0 : plan->biased_byte;
    plan->plane_base = 0x01000000U;
    plan->glyph_attr = 0xc000U;
    /* or (like 0x1cfe0, unlike the 0x1cf40 setbit): both attribute
     * bits are forced onto the glyph data. */
