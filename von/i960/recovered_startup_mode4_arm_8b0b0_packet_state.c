/* Selector-2 post-dispatch packet/state sequence recovered from i960 0x8b0b0-0x8b1a4. */
#include "recovered_common.h"

typedef struct {
    recovered_u32 state_51c940;
    recovered_u32 state_51c948;
    recovered_u32 state_51c94c;
    recovered_u32 command_29_packet[3];
    recovered_u32 command_30_packet[3];
    recovered_u32 command_10_packet[3];
    recovered_u32 command_31_packet[6];
    recovered_u32 first_response;
    recovered_u32 second_response;
    recovered_u32 command_10_response;
    recovered_u32 command_31_response;
    recovered_u32 state_51c950;
    recovered_u32 state_51c954;
    recovered_u32 rolling_51c958;
    recovered_u32 rolling_51c95c;
    recovered_u32 rolling_51c960;
    recovered_u32 fifo_address;
    recovered_u32 continuation;
} recovered_startup_mode4_arm_8b0b0_packet_state_result;

recovered_startup_mode4_arm_8b0b0_packet_state_result
recovered_startup_mode4_arm_8b0b0_packet_state(
    recovered_u32 state_51c940, recovered_u32 state_51c948,
    recovered_u32 state_51c94c, recovered_u32 command_10_word_1,
    recovered_u32 command_10_word_2, recovered_u32 command_31_word_1,
    recovered_u32 command_31_word_2, recovered_u32 command_31_word_3,
    recovered_u32 command_31_word_4, recovered_u32 command_31_word_5,
    recovered_u32 first_response, recovered_u32 second_response,
    recovered_u32 command_10_response, recovered_u32 command_31_response,
    recovered_u32 state_51c950, recovered_u32 state_51c954,
    recovered_u32 rolling_51c958, recovered_u32 rolling_51c95c,
    recovered_u32 rolling_51c960)
{
    recovered_startup_mode4_arm_8b0b0_packet_state_result result;

    result.state_51c940 = state_51c940;
    result.state_51c948 = state_51c948;
    result.state_51c94c = state_51c94c;
    result.command_29_packet[0] = 29U;
    result.command_29_packet[1] = state_51c940 & 0xffffU;
    result.command_29_packet[2] = state_51c948;
    result.command_30_packet[0] = 30U;
    result.command_30_packet[1] = state_51c940 & 0xffffU;
    result.command_30_packet[2] = state_51c948;
    result.command_10_packet[0] = 10U;
    result.command_10_packet[1] = command_10_word_1;
    result.command_10_packet[2] = command_10_word_2;
    result.command_31_packet[0] = 31U;
    result.command_31_packet[1] = command_31_word_1;
    result.command_31_packet[2] = command_31_word_2;
    result.command_31_packet[3] = command_31_word_3;
    result.command_31_packet[4] = command_31_word_4;
    result.command_31_packet[5] = command_31_word_5;
    result.first_response = first_response;
    result.second_response = second_response;
    result.command_10_response = command_10_response;
    result.command_31_response = command_31_response;
    result.state_51c950 = state_51c950;
    result.state_51c954 = state_51c954;
    result.rolling_51c958 = rolling_51c958;
    result.rolling_51c95c = rolling_51c95c;
    result.rolling_51c960 = rolling_51c960;
    result.fifo_address = RECOVERED_FIFO_ADDRESS;
    result.continuation = 0x0008b1a4U;
    return result;
}
