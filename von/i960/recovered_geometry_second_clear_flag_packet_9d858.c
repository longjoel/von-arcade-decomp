/* Second clear-flag geometry packet recovered from i960 0x9d858-0x9d9a0. */
#include <stdint.h>
#include "recovered_common.h"

typedef uint32_t u32;

struct recovered_geometry_second_clear_flag_packet_plan {
    int16_t object_1e2;
...
void recovered_geometry_second_clear_flag_packet_plan(
    int16_t object_1e2, u32 derived_packet_word, u32 object_flag_1dd,
    u32 frame_value, u32 board_readback_address,
    struct recovered_geometry_second_clear_flag_packet_plan *plan)
{
    plan->object_1e2 = object_1e2;
    ...
    if (plan->object_flag_1dd == 0U) {
        plan->frame_variant = 0U;
        plan->frame_slot_offset = 0x50U;
        plan->frame_word[0] = frame_value;
        plan->frame_word[1] = 0x40005cU;
        plan->frame_tail_offset = 0x58U;
    } else {
        plan->frame_variant = 1U;
        plan->frame_slot_offset = 0x60U;
        plan->frame_word[0] = frame_value;
        plan->frame_word[1] = 0x40002cU;
        plan->frame_tail_offset = 0x68U;
    }
}
