...
source = root / "von/i960/recovered_geometry_diagnostic_math_post_response_gate_8fb0c.c"
listing = (root / "von/build/disasm/vonj-maincpu.lst").read_text()
for address, text in (("8fb04", "ld"), ("8fb0c", "cmpi"),
                      ("8fb1c", "st"), ("8fb24", "ld"),
                      ("8fb2c", "bne")):
...
with tempfile.TemporaryDirectory() as directory:
    ...
    function = ctypes.CDLL(str(library)).recovered_geometry_diagnostic_math_post_response_gate_8fb0c
    function.argtypes = [ctypes.POINTER(Input), ctypes.POINTER(Plan)]
    sample = Input(0, 0x7000, 0xa1b2c3d4)
    plan = Plan()
    ...
    assert (plan.frame_readback, plan.published_address, plan.published_value,
            plan.fifo_address, plan.fifo_response, plan.selected_target) == (
                0x7000, 0x801008, 0x7034, 0x884000, 0xa1b2c3d4, 0x8fb30)
    sample.state_flag = 1
    function(ctypes.byref(sample), ctypes.byref(plan))
    assert plan.selected_target == 0x8fb84

print("recovered geometry 0x8fb0c post-service response fixture: ok")
