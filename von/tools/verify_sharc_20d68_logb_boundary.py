#!/usr/bin/env python3
"""Verify the exact LOGB exponent-distance comparator boundary."""
...
# (helper F0/F1, trace length, path marker, rounded F15 at 0x20db4)
EXPECTED = (
    (("7d000000", "3f800000"), 48, "020d81", "3fc90fdb"),
    (("3f800000", "7d000000"), 40, "020d89", "02000000"),
    (("7d800000", "3f800000"), 15, "020db8", "3fc90fdb"),
    (("3f800000", "7d800000"), 20, "020db5", "00000000"),
    (("7e000000", "3f800000"), 15, "020db8", "3fc90fdb"),
    (("3f800000", "7e000000"), 20, "020db5", "00000000"),
...
def main() -> int:
    ...
    for number, start in enumerate(starts[:len(EXPECTED)], 1):
        ...
        if (call[0].get("f0"), call[0].get("f1")) != expected_input:
            raise SystemExit(f"LOGB-boundary call {number} has unexpected input")
        if len(call) != expected_length or marker not in {record.get("pc") for record in call}:
            raise SystemExit(f"LOGB-boundary call {number} took an unexpected path")
        result = next((record for record in call if record.get("pc") == "020db4"), None)
        if result is None or result.get("f15") != expected_output:
            raise SystemExit(f"LOGB-boundary call {number} has unexpected result")

    print("PASS: SHARC helper-0x20d68 exact LOGB boundary trace")
    return 0
