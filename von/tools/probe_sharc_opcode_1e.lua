-- Probe the two-input cosine-times-float service at opcode 0x1e.

local frame = 0
local space
local log_file
local requests = {
    { 0x00000000, 0x3f800000 }, -- cos(0) * 1.0
    { 0x00004000, 0x40000000 }, -- cos(pi/2) * 2.0
    { 0x00002000, 0x3f800000 }, -- cos(pi/4)
    { 0xffffc000, 0x3f800000 }, -- cos(-pi/2)
}
...
local function send(angle, multiplier)
    ...
    space:write_u32(0x00884000, 0x0000001e)
    space:write_u32(0x00884000, angle)
    space:write_u32(0x00884000, multiplier)
    log(string.format("probe: opcode=0x1e angle=0x%08x multiplier=0x%08x", angle, multiplier))
end
...
emu.register_periodic(function()
    ...
    if not space then
        ...
        if cpu then
            ...
            log_file = assert(io.open(os.getenv("VON_SHARC_1E_LOG") or
                "von-sharc-opcode-1e.log", "w"))
            log("probe: start")
        end
    end
