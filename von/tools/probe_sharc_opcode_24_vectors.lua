-- Exercise opcode 0x24 after loading a fresh identity affine state.
local frame = 0
local space
local log_file
...
emu.register_periodic(function()
    ...
    if not space then
        ...
        if cpu then
            ...
            log_file = assert(io.open(os.getenv("VON_SHARC_24_LOG") or
                "von-sharc-opcode-24-vectors.log", "w"))
            log("probe: start")
        end
    end
