#!/usr/bin/env python3
"""Validate the secondary slot-20 mode-7 continuation."""
import ctypes
import pathlib
import subprocess
import tempfile
...
SOURCE = ROOT / "von/i960/recovered_startup_mode4_arm_878a4_mode7_continuation.c"
...
def main():
    with tempfile.TemporaryDirectory() as directory:
        ...
        fn = ctypes.CDLL(str(library)).recovered_startup_mode4_arm_878a4_mode7_continuation
        fn.argtypes = [ctypes.c_uint32, ctypes.POINTER(Result)]
        fn.restype = ctypes.c_int
        result = Result()
        assert fn(0x89abcdef, ctypes.byref(result)) == 1
        assert result.mode == 7 and result.flag == 1
    ...
    print("PASS: 0x878a4 mode-7 slot-20 continuation")
