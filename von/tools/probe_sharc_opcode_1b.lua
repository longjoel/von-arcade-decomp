-- Probe the signed-fixed-point conversion service at opcode 0x1b.
...
local function send(value)
    ...
    space:write_u32(0x00884000, 0x0000001b)
    space:write_u32(0x00884000, value)
    log(string.format("probe: opcode=0x1b input=0x%08x", value))
end
...
emu.register_periodic(function()
    ...
    if not space then
        ...
        if cpu then
            ...
            log_file = assert(io.open(os.getenv("VON_SHARC_1B_LOG") or
                "von-sharc-opcode-1b.log", "w"))
            log("probe: start")
        end
    end
