#!/usr/bin/env python3
"""Validate the recovered 0x1cf40 plane-1 glyph emitter plan."""
import ctypes
import pathlib
import subprocess
import tempfile
...
with tempfile.TemporaryDirectory() as td:
    so = pathlib.Path(td) / "glyph-emit-plane1.so"
    subprocess.run(["cc", "-shared", "-fPIC", "-O2",
                    str(pathlib.Path(__file__).parents[1] / "i960" /
                        "recovered_glyph_emit_plane1_1cf40.c"),
                    "-o", str(so)], check=True)
    lib = ctypes.CDLL(str(so))
    plan_fn = lib.recovered_glyph_emit_plane1_plan
    plan_fn.argtypes = [ctypes.c_uint32, ctypes.c_uint32,
                        ctypes.c_uint32, ctypes.POINTER(Plan)]

    # Printable 'A' renders table glyph 33 in plane 1 with bit15 only.
    plan = Plan()
    ...
    assert (plan.glyph_table, plan.table_index, plan.plane_base,
            plan.glyph_attr) == (0x02EA0FD0, 33, 0x01002000, 0x8000)
    assert list(plan.tile_addresses) == [0x01002610, 0x01002690]
    assert (plan.column_wrap, plan.next_column) == (61, 9)

    # No control-code branches: biased 0x4b/0x54 stay on the table path.
    plan_fn(0x6B, 8, 12, ctypes.byref(plan))
    assert (plan.biased_byte, plan.zeroed, plan.table_index) == (
        0x4B, 0, 0x4B)
...
print("PASS: 0x1cf40 glyph emitter plan")
