-- Seed an asymmetric matrix, then exercise opcode 0x32's three angle fields.
...
emu.register_periodic(function()
    ...
    if not space then
        ...
        if cpu then
            ...
            log_file = assert(io.open(os.getenv("VON_SHARC_32_ASYMMETRIC_LOG") or
                "vonj-sharc-opcode-32-asymmetric.log", "w"))
    ...
    elseif frame == 780 then
        header(0x32)
        for _, value in ipairs({0, 0, 0, 0x4000, 0, 0, 0, 0x4000, 0x4000}) do word(value) end
        log_file:write("probe: angles=R3,R5,R6=0x4000\n"); log_file:flush()
    end
    if frame >= 1100 then
