#!/usr/bin/env python3
"""Vectors for the fixed 0x88200 row-value accessor."""
...
SOURCE = ROOT / "i960/recovered_stage_row_value_accessor_88200.c"
...
def main():
    ...
    for instruction in (r"88200:.*lda.*0x88240",
                        r"88208:.*mov.*g14,g1",
                        r"88210:.*ld.*0x51c988",
                        r"88218:.*cmpibg.*0.*0x88228",
                        r"8821c:.*lda.*0xffff",
                        r"88228:.*ld.*0x51d5e4",
                        r"88230:.*lda.*\(g4\)\[g4\*2\]",
                        r"88234:.*ldos.*0x5618f0\[g4\*4\]",
                        r"8823c:.*bx.*\(g1\)"):
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        subprocess.run(["cc", "-std=c11", "-Wall", "-Wextra", "-Werror",
                       ...
                       check=True)
        function = ctypes.CDLL(str(library)).recovered_stage_row_value_accessor_88200
        ...
        assert (positive.row_offset, positive.read_address,
                positive.result_value, positive.table_stride) == (84, 0x561944, 0x5678, 12)
        ...
        assert (positive.return_trampoline_load_address,
                positive.return_trampoline_target, positive.continuation) == (0x88200, 0x88240, 0x88240)
    print("recovered 0x88200 stage-row-accessor vectors: ok")
