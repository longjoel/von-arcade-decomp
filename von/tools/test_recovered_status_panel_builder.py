...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="von-status-panel-builder-") as directory:
        ...
        plan = Plan()
        plan_fn(0x130, 0xFFFFFFE6, ctypes.byref(plan))
        ...
        plan_fn(0x13F, 0xFFFFFFE6, ctypes.byref(plan))
        assert (plan.second_selector, plan.second_table_entry) == (0xF, 0x02EA204C)
        plan_fn(0x00000000, 0xFFFFFFFF, ctypes.byref(plan))
        assert plan.second_selector == 0
        assert plan.first_width == 30
