...
def main():
 with tempfile.TemporaryDirectory() as temp:
  ...
  subprocess.run(['python3',TOOL,'--fingerprints',fp,'--family','family-00','--trace',trace,'--rom',rom,'--output',out,'--root',root],check=True)
  ...
  outside=root.parent/'outside-family-animation.gltf'
  result=subprocess.run(['python3',TOOL,'--fingerprints',fp,'--family','family-00','--trace',trace,'--rom',rom,'--output',outside,'--root',root],capture_output=True,text=True)
