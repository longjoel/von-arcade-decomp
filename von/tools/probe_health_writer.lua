...
-- Deferred install: frame-1 taps are discarded by a boot-time memory
-- reconfiguration, so arm the writer-PC taps only after the machine has
-- settled. Default 1400 matches the documented working threshold.
local WATCH_INSTALL_FRAME = tonumber(os.getenv("VON_WATCH_INSTALL_FRAME") or "1400") or 1400
local WATCH_INSTALLED = false
local WATCH_DETAIL = os.getenv("VON_WATCH_DETAIL") == "1"
...
local function cpu_reg(name)
    if not cpu_dev then return nil end
    for _, n in ipairs({ name, string.upper(name) }) do
        local ok, v = pcall(function() return cpu_dev.state[n].value end)
...
local function rdh(addr)
    local ok, v = pcall(function() return space:read_u16(addr) end)
    return ok and v or nil
end

local function fmt16(v) return v and string.format("%04x", v) or "?" end
local function fmt32(v) return v and string.format("%08x", v) or "?" end
            ...
            return space:install_write_tap(w.base, w.base + w.len - 1,
                ...
                function(offset, data, mask)
                    ...
                    if seen and cpu_dev then
                        ...
                        if okc and type(st) == "number" then curpc = st end
                        local pc = curpc and string.format("0x%x", curpc) or "?"
                        if not seen[pc] then
                        ...
                        -- Detail on the health-applier store (static store at
                        -- 0x33840, MAME reports CURPC one instruction later).
                        if WATCH_DETAIL and curpc == 0x33844 then
                            local r8 = cpu_reg("r8")
                            local r4 = cpu_reg("r4")
                            local g0 = cpu_reg("g0")
                            local prof = rdw(0x51ab14)
                            local extra = ""
                            if r8 and rdw(r8 + 0x6c) then
                                local mech = rdw(r8 + 0x6c)
                                extra = string.format(
                                    " obj64=%s obj4a=%s obj46=%s obj172=%s obj190=%s obj192=%s obj1d0=%s obj1d2=%s obj1d8=%s prof=%08x p63c=%s p640=%s p3e4=%s p3e8=%s p3ec=%s",
                                    fmt16(rdh(r8 + 0x64)), fmt16(rdh(r8 + 0x4a)),
                                    fmt16(rdh(r8 + 0x46)), fmt16(rdh(r8 + 0x172)),
                                    fmt16(rdh(r8 + 0x190)), fmt16(rdh(r8 + 0x192)),
                                    fmt16(rdh(r8 + 0x1d0)), fmt16(rdh(r8 + 0x1d2)),
                                    fmt16(rdh(r8 + 0x1d8)), mech,
                                    fmt16(rdh(mech + 0x63c)), fmt16(rdh(mech + 0x640)),
                                    fmt16(rdh(mech + 0x3e4)), fmt16(rdh(mech + 0x3e8)),
                                    fmt16(rdh(mech + 0x3ec)))
                            end
                            log(string.format(
                                "healthstore f%d @%08x data=%08x mask=%08x r4=%s r8=%s g0=%s prof=%s%s",
                                frame, offset, data, mask or 0, fmt32(r4), fmt32(r8),
                                fmt32(g0), fmt32(prof), extra))
                        end
                    end
                    return data
                end)
...
emu.register_periodic(function()
    ...
    if space and not WATCH_INSTALLED and frame >= WATCH_INSTALL_FRAME then
        WATCH_INSTALLED = true
        log(string.format("watch: deferred install at f%d", frame))
        install_watches()
        install_struct_field_watch()
    end
