-- Probe opcode 0x3d's edge vectors, reading back the complete state after
...
emu.register_periodic(function()
    ...
    if not space then
        ...
        if cpu then
            ...
            log_file = assert(io.open(os.getenv("VON_SHARC_3D_EDGES_LOG") or
                "von-sharc-opcode-3d-edges.log", "w"))
            log("probe: start")
        end
    end
