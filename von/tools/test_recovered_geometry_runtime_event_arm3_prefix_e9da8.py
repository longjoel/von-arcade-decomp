...
def main():
    with tempfile.TemporaryDirectory(prefix="von-event-arm3-") as d:
        so = Path(d) / "event-arm3.so"
        subprocess.run([os.environ.get("CC", "cc"), "-shared", "-fPIC", "-O2",
                        "-I", str(ROOT / "von/i960"), "-o", str(so),
                        str(ROOT / "von/i960/recovered_geometry_runtime_event_arm3_prefix_e9da8.c")], check=True)
        lib = ctypes.CDLL(str(so))
        fn = lib.recovered_geometry_runtime_event_arm3_prefix_e9da8
        ...
        assert (out.delta_word_10, out.delta_word_8, out.state_3e4,
                out.threshold, out.extended_path) == (0xffffff80, 0x80, 0x12345678, 44, 0)
        assert list(out.packet) == [10, 0xffffff80, 0x80]
        out = fn(0, 0, 0, 0, 45, 7)
        assert out.extended_path == 1
        assert (out.short_path_target, out.extended_path_target,
                out.state_address, out.fifo_address) == (0xe9e00, 0xe9ec0, 0x5783e4, 0x884000)
        print("PASS: 0xe9da8 runtime event arm 3 prefix")
