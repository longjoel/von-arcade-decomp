#!/usr/bin/env python3
"""Test the paired renderer route at 0x1f1b0."""
...
SOURCE = ROOT / "von/i960/recovered_text_status_panel2_route.c"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="von-status-panel2-") as directory:
        library = Path(directory) / "status-panel2.so"
        subprocess.run([os.environ.get("CC", "cc"), "-shared", "-fPIC", "-O2", SOURCE, "-o", library], check=True)
        recovered = ctypes.CDLL(str(library))
        plan_fn = recovered.recovered_text_status_panel2_plan
        ...
        assert (plan.helper, plan.source, plan.column, plan.row, plan.width,
                plan.height) == (0x1DD10, 0x02FD832E, 2, 35, 43, 3)
    ...
    print("PASS: 0x1f1b0 status-panel source/fill route")
    return 0
