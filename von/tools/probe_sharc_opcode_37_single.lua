-- Isolate one opcode 0x37 tail field and read back the reset state.

local frame = 0
local space
local log_file
local field = os.getenv("VON_SHARC_37_FIELD") or "R13"
local values = {
    R13 = {0x3f800000, 0, 0},
    R14 = {0, 0x40000000, 0},
    R15 = {0, 0, 0x40400000},
...
emu.register_periodic(function()
    ...
    if not space then
        ...
        if cpu then
            ...
            log_file = assert(io.open(os.getenv("VON_SHARC_37_LOG") or
                "vonj-sharc-opcode-37-single.log", "w"))
