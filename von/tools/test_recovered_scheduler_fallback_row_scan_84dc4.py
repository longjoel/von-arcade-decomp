...
SOURCE = ROOT / "von/i960/recovered_scheduler_fallback_row_scan_84dc4.c"
LISTING = ROOT / "von/build/disasm/vonj-maincpu.lst"


class Scan(ctypes.Structure):
    _fields_ = [(name, ctypes.c_uint32) for name in (
        "success_84dac", "continues_84f10", "selected_row", "scanned_rows")]


with tempfile.TemporaryDirectory() as directory:
    library = pathlib.Path(directory) / "fallback-scan.so"
    subprocess.run(["cc", "-shared", "-fPIC", "-O2", "-o", str(library),
                    str(SOURCE)], check=True)
    lib = ctypes.CDLL(str(library))
    build = lib.recovered_scheduler_fallback_row_scan_84dc4
    words = ctypes.c_uint32 * 8
    build.argtypes = [words, words, words]
    build.restype = Scan

    fields = words(60, 60, 60, 60, 60, 60, 60, 60)
    globals_ = words(0, 1, 1, 1, 0, 0, 0, 0)
    counts = words(2, 2, 3, 2, 2, 2, 2, 2)
    result = build(fields, globals_, counts)
    assert (result.success_84dac, result.continues_84f10,
    ...
    terminal_counts = words(2, 2, 2, 2, 2, 2, 2, 3)
    result = build(terminal_fields, terminal_globals, terminal_counts)
    assert (result.success_84dac, result.selected_row,
            result.scanned_rows) == (1, 7, 8)

print("recovered 0x84dc4 fallback-row-scan vectors: ok")

listing = [" ".join(line.split()).lower()
           for line in LISTING.read_text(encoding="utf-8").splitlines()]
for address, instruction in (
    ("84dc4:", "mov 0,g13"),
    ("84e34:", "ldos 0x86(g5),g4"),
    ("84ef4:", "cmpibge 2,g6,0x84efc"),
    ("84efc:", "addo g13,1,g13"),
    ("84f00:", "cmpi 7,g13"),
    ("84f08:", "lda 0x90(g3),g3"),
):
    assert any(address in line and instruction in line for line in listing), (address, instruction)

print("recovered 0x84dc4 fallback-row-scan listing evidence: ok")
